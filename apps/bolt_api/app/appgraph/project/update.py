...
class Update(UpdateValidate):
    ...
    def process_upload(image_url) -> str:
        ...
        src_bucket = current_app.app_config.get('BUCKET_PRIVATE_STORAGE', None)
        if src_bucket not in image_url:
            return image_url
        upload_id = image_url.split('?')[0].rsplit('/')[-1]
        uploads.process_image(current_app.config, upload_id, upload_id)
        return uploads.get_object_public_url(current_app.config, upload_id)
