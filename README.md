...
* `config.BUCKET_PRIVATE_STORAGE` - to temporarily keep uploaded files before processing
* `config.BUCKET_PUBLIC_UPLOADS` - to eventually store processed uploaded files in a publicly accessible location.
...
* `http://storegadomain.com/<config.BUCKET_PUBLIC_UPLOADS>/RESOURCE_NAME` - image in default thumbnail size (`thumbnails.DEFAULT_SIZE`)
* `http://storegadomain.com/<config.BUCKET_PUBLIC_UPLOADS>/RESOURCE_NAME/original` - original image in it's full size, if file size does not exceed `thumbnails.MAX_ORIGINAL_BYTES`
* `http://storegadomain.com/<config.BUCKET_PUBLIC_UPLOADS>/RESOURCE_NAME/800x300` - image thumbnail that's 800px wide and 300px tall
* `http://storegadomain.com/<config.BUCKET_PUBLIC_UPLOADS>/RESOURCE_NAME/<WWW>x<HHH>` - and so forth for each of `thumbnail.SIZES`
